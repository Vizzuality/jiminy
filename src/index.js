...
export default class Jiminy {
  ...
  recommendation(fieldNames) {
    ...
    return this._availableCharts.map(chart => chart.name);
  }
  ...
  columns(chartName, columnName) {
    ...
    if (!columnName) {
      return chart.computeUsefulFields(this._fields.fields).map(field => field.name);
    }
    ...
    return chart.computeUsefulFields(this._fields.fields, field).map(field => field.name);
  }
}
